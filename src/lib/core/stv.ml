...
(* ============== WM: utilities for debugging ==================== *)
(* Added cohttp-lwt-unix to dune libraries *)
(* open Lwt.Syntax
open Lwt_unix *)
...
let pp_int_list fmt ls =
  ...
  List.iteri (fun i x ->
    ...
    Format.fprintf fmt "%d" x) ls;
  Format.fprintf fmt "]"

let pp_list_of_int_list fmt lls =
  Format.fprintf fmt "[";
  List.iteri (fun i x ->
    if i > 0 then Format.fprintf fmt "; ";
    Format.fprintf fmt "%a" pp_int_list x) lls;
  Format.fprintf fmt "]"
...
let pp_int_array fmt arr =
  ...
  Array.iteri (fun i x ->
    if i > 0 then Format.fprintf fmt "; ";
    Format.fprintf fmt "%d" x) arr;
  Format.fprintf fmt "|]"

let pp_array_of_int_array fmt arr =
  Format.fprintf fmt "[|";
  Array.iteri (fun i x ->
    if i > 0 then Format.fprintf fmt "; ";
    Format.fprintf fmt "%a" pp_int_array x) arr;
  Format.fprintf fmt "|]"

let pp_polyvar_list fmt pls =
  Format.fprintf fmt "[";
  List.iteri (fun i x ->
    if i > 0 then Format.fprintf fmt "; ";
    match x with
    | `Win l -> Format.fprintf fmt "`Win %a" pp_int_list l
    | `TieWin l -> Format.fprintf fmt "`TieWin %a" pp_int_list l
    | `TieLose l -> Format.fprintf fmt "`TieLose %a" pp_int_list l
    | `Lose i -> Format.fprintf fmt "`Lose %d" i) pls;
  Format.fprintf fmt "]"

let pp_tuple fmt t =
  match t with
  | (f, l) ->
    Format.fprintf fmt "(%f, %a)" f pp_int_list l

let pp_tuple_list fmt ls =
  Format.fprintf fmt "[";
  List.iteri (fun i x ->
    if i > 0 then Format.fprintf fmt "; ";
    Format.fprintf fmt "%a" pp_tuple x) ls;
  Format.fprintf fmt "]"
(* ================================================================ *)
...
let compute ~nseats ballots =
  ...
  in
  let _ = Lwt_fmt.(fprintf stderr "%s\n" "=================== In: compute function log =================== ") in      (*WM*)
  let _ = Lwt_fmt.(fprintf stderr "nseats: %d\n" nseats) in                                                           (*WM*)
  let _ = Lwt_fmt.(fprintf stderr "nballots: %d\n" nballots) in                                                       (*WM*)
  let _ = Lwt_fmt.(fprintf stderr "ballots:\n%a\n" pp_array_of_int_array ballots) in                                  (*WM*)
  let _ = Lwt_fmt.(fprintf stderr "stv_ballots:\n%a\n" pp_list_of_int_list stv_ballots) in                            (*WM*)
  let _ = Lwt_fmt.(fprintf stderr "stv_invalid:\n%a\n" pp_array_of_int_array stv_invalid) in                          (*WM*)
  let _ = Lwt_fmt.(fprintf stderr "quota: %f\n" quota) in                                                             (*WM*)
  let _ = Lwt_fmt.(fprintf stderr "wballots: \n%a\n" pp_tuple_list wballots) in                                       (*WM*)
  let _ = Lwt_fmt.(fprintf stderr "stv_events:\n%a\n" pp_polyvar_list stv_events) in                                  (*WM*)
  let _ = Lwt_fmt.(fprintf stderr "stv_winners:\n%a\n" pp_int_list stv_winners) in                                    (*WM*)  
  let _ = Lwt_fmt.(fprintf stderr "%s\n@." "=================== End: compute function log =================== ") in   (*WM*)
